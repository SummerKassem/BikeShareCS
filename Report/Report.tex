...
\item \textit{mean\_max\_by\_month}:\\	
	The method \textit{mean\_max\_by\_month} returns a pivot table whose index is the month, and the columns are the mean and maximum ride length for each rider type. The method also aggregates the mean and max values across the entire year. The pivot table is shown in the top of Figure \ref{fig11_1}, and the corresponding visualisations are shown on the bottom Figure \ref{fig11_2}.
\pagebreak
	\begin{figure}[h]
	\centering
...
\end{table}
	...
	The method \textit{count\_rideable\_type} 
	\begin{figure}[h]
	\centering
	\includegraphics[scale=0.6]{rideable_types.pdf} 
	\caption{rideable types}
	\label{fig16}
	\end{figure}
